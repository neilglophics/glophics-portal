...
Router.register("tickets", {
  ...
  render() {
    ...
    return H.page(`
      ...
      <section class="grid grid-cols-1 gap-4 pb-5 md:grid-cols-3">
        ${top[0] ? H.statTile("info", this.titleCase(top[0]), byStatus[top[0]], "tickets occupying", "check")
                 : H.statTile("info", "Occupying", 0, occupying.join(", ") || "no statuses set", "check")}
        ${top[1] ? H.statTile("alt", this.titleCase(top[1]), byStatus[top[1]], "tickets occupying", "check")
                 : H.statTile("alt", "People holding", s.people, "across all accounts", "check")}
  ...
  row({ claim, env, accountName, minutesLeft }) {
    return H.tr(
      ...
      H.td(`<span class="line-clamp-1 block max-w-[260px] text-sm text-body">${H.esc(claim.summary || claim.note || "—")}</span>`) +
